# -*- coding: utf-8 -*-

"""
    smsapi.models.message_status

    This file was automatically generated by APIMATIC v2.0 ( https://apimatic.io )
"""


class MessageStatus(object):

    """Implementation of the 'MessageStatus' model.

    TODO: type model description here.

    Attributes:
        to (string): The phone number (recipient) the message was sent to.
        received_timestamp (string): The date and time when the message was
            recieved by recipient.
        sent_timestamp (string): The date and time when the message was sent.
        status (StatusEnum): Delivery status of the message

    """

    # Create a mapping from Model property names to API property names
    _names = {
        "to" : "to",
        "received_timestamp" : "receivedTimestamp",
        "sent_timestamp" : "sentTimestamp",
        "status" : "status"
    }

    def __init__(self,
                 to=None,
                 received_timestamp=None,
                 sent_timestamp=None,
                 status=None):
        """Constructor for the MessageStatus class"""

        # Initialize members of the class
        self.to = to
        self.received_timestamp = received_timestamp
        self.sent_timestamp = sent_timestamp
        self.status = status


    @classmethod
    def from_dictionary(cls,
                        dictionary):
        """Creates an instance of this model from a dictionary

        Args:
            dictionary (dictionary): A dictionary representation of the object as
            obtained from the deserialization of the server's response. The keys
            MUST match property names in the API description.

        Returns:
            object: An instance of this structure class.

        """
        if dictionary is None:
            return None

        # Extract variables from the dictionary
        to = dictionary.get("to")
        received_timestamp = dictionary.get("receivedTimestamp")
        sent_timestamp = dictionary.get("sentTimestamp")
        status = dictionary.get("status")

        # Return an object of this model
        return cls(to,
                   received_timestamp,
                   sent_timestamp,
                   status)


